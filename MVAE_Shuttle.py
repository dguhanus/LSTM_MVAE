...
dfdata = loadmat('data/shuttle.mat')
...
mn = ['Column'+ str(i) for i in range(0,10)]
X11 = pd.DataFrame(X11[:,0:10], columns=mn)
X11.rename(columns={'Column9': 'Class'},inplace = True)
...
anamoly_dataset = anamoly_dataset.sample(frac=0.4)
...
#from DeepVAEMSE import create_lstm_vae
...
for i in val_range:
    threshold = error_df.reconstruction_error.mean()  +  0.9 * i * error_df.reconstruction_error.std()
